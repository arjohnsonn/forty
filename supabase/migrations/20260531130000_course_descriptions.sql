...
-- Catalog course description (scraped from the course-schedule detail page; see scripts/scrape.ts
-- `descriptions` + scripts/descriptions.ts). Course-level, so it lives on courses.
ALTER TABLE public.courses ADD COLUMN IF NOT EXISTS description text;

-- Recreate match_sections_detailed (was last defined in 20260531000000_rmp.sql) to also return the
-- course description, so the chat Worker passes it through to the answer + the course card.
